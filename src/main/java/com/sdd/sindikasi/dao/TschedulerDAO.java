...
public class TschedulerDAO {
	...
	public List<Tscheduler> listPaging(int first, int second, String filter, String orderby) throws Exception {
		...
		Query query = session.createQuery("from Tscheduler where " + filter + " order by " + orderby);
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Tscheduler " + "where " + filter)
	...
	public Tscheduler findByPk(Integer pk) throws Exception {
		session = StoreHibernateUtil.openSession();
		Tscheduler oForm = (Tscheduler) session.createQuery("from Tscheduler where tschedulerpk = " + pk)
				.uniqueResult();
		session.close();
		return oForm;
	}
	...
	public Tscheduler findById(String id) throws Exception {
		session = StoreHibernateUtil.openSession();
		Tscheduler oForm = (Tscheduler) session.createQuery("from Tscheduler where schedulerid = '" + id + "'")
				.uniqueResult();
		session.close();
		return oForm;
	}
