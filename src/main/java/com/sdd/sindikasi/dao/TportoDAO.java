...
public class TportoDAO {
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Tporto " + "where " + filter)
	...
	public List<Vsummary> listSummary(String filter) throws Exception {
		...
		oList = session.createSQLQuery(
				"SELECT count(*) as totaldata,  coalesce(sum(projectamount),0)  as amounttotal FROM TPIPELINE WHERE "
						+ filter)
