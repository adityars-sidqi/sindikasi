...
public class MusergroupmenuVm {
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
		...
		try {
			this.obj = obj;
			List<Musergroupmenu> objList = oDao.listByFilter(
					"musergroup.musergrouppk = " + obj.getMusergrouppk(),
					"musergroupmenupk");
			for (Musergroupmenu data : objList) {
				map.put(data.getMmenu().getMmenupk(),
						data.getMmenu());
			}
		...
		try {
			...
			for (Mmenu menu: listMenu) {
				if (!menugroup.equals(menu.getMenugroup())) {					
					...
					lblGroup.setStyle("font-weight: bold");
					lblGroup.setParent(divListMenu);*/
					grid = new Grid();
					...
					chk.addEventListener(Events.ON_CHECK,
				...
				chk.addEventListener(Events.ON_CHECK,
						new EventListener<Event>() {
							...
							public void onEvent(Event event)
									throws Exception {
								if (chk.isChecked()) {
									listMmenu.add((Mmenu) chk
											.getAttribute("obj"));
								} else {
									listMmenu.remove(chk
											.getAttribute("obj"));
								}
							}
						});
	...
	public void save() {
		try {
			...
			transaction.commit();
			session.close();
			Clients.showNotification("Pengaturan Menu Sukses", "info",
