...
import com.sdd.sindikasi.model.MrmgroupListModel;
import com.sdd.utils.SysUtils;
import com.sdd.utils.db.StoreHibernateUtil;

public class MrmgroupVm {
	...
	private MrmgroupListModel model;
	private MrmgroupDAO oDao = new MrmgroupDAO();
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Mrmgroup>() {

				public void render(Listitem item, final Mrmgroup data, int index) throws Exception {
					Listcell cell = new Listcell(String.valueOf((SysUtils.PAGESIZE * pageStartNumber) + index + 1));
					item.appendChild(cell);
					cell = new Listcell(data.getRmgroupcode());
					item.appendChild(cell);
					cell = new Listcell(data.getRmgroupname());
					item.appendChild(cell);
	...
	public void refreshModel(int activePage) {
		...
		model = new MrmgroupListModel(activePage, SysUtils.PAGESIZE, filter, orderby);
		if (needsPageUpdate) {
	...
	public void doSearch() {
		filter = "";
		if (rmgroupcode != null && rmgroupcode.trim().length() > 0) {
			if (filter.length() > 0)
				filter += " and ";
			filter += "rmgroupcode like '%" + rmgroupcode.trim().toUpperCase() + "%'";
		}
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									BindUtils.postNotifyChange(null, null, MrmgroupVm.this, "objForm");
									doReset();
								} catch (HibernateException e) {
									transaction.rollback();
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				...
				String rmgroupcode = (String) ctx.getProperties("rmgroupcode")[0].getValue();
				String rmgroupname = (String) ctx.getProperties("rmgroupname")[0].getValue();

				if (rmgroupcode == null || "".equals(rmgroupcode.trim()))
					this.addInvalidMessage(ctx, "rmgroupcode", Labels.getLabel("common.validator.empty"));
				if (rmgroupname == null || "".equals(rmgroupname.trim()))
					this.addInvalidMessage(ctx, "rmgroupname", Labels.getLabel("common.validator.empty"));
			}
		};
	}

	public Mrmgroup getObjForm() {
		return objForm;
	}

	public void setObjForm(Mrmgroup objForm) {
