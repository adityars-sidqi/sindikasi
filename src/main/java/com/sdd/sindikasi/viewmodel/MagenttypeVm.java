...
import com.sdd.sindikasi.model.MagenttypeListModel;
import com.sdd.utils.SysUtils;
import com.sdd.utils.db.StoreHibernateUtil;

public class MagenttypeVm {
	...
	private MagenttypeListModel model;
	private MagenttypeDAO oDao = new MagenttypeDAO();
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		if (listbox != null) {
			listbox.setItemRenderer(new ListitemRenderer<Magenttype>() {

				public void render(Listitem item, final Magenttype data, int index) throws Exception {
					Listcell cell = new Listcell(String.valueOf((SysUtils.PAGESIZE * pageStartNumber) + index + 1));
					item.appendChild(cell);
					cell = new Listcell(data.getAgentcode());
					item.appendChild(cell);
	...
	public void refreshModel(int activePage) {
		...
		model = new MagenttypeListModel(activePage, SysUtils.PAGESIZE, filter, orderby);
		if (needsPageUpdate) {
	...
	public void doSearch() {
		...
		if (agentcode != null && agentcode.trim().length() > 0) {
			if (filter.length() > 0)
				filter += " and ";
			filter += "agentcode like '%" + agentcode.trim().toUpperCase() + "%'";
		}
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									doReset();
									BindUtils.postNotifyChange(null, null, MagenttypeVm.this, "objForm");
								} catch (HibernateException e) {
									transaction.rollback();
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String agentcode = (String) ctx.getProperties("agentcode")[0].getValue();
				String description = (String) ctx.getProperties("description")[0].getValue();

				if (agentcode == null || "".equals(agentcode.trim()))
					this.addInvalidMessage(ctx, "agentcode", Labels.getLabel("common.validator.empty"));
	...
	public Magenttype getObjForm() {
		return objForm;
	}

	public void setObjForm(Magenttype objForm) {
