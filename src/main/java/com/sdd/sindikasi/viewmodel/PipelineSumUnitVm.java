...
public class PipelineSumUnitVm {
	
	private Vpipelineunit objForm;
	private List<Vpipelineunit> objList;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		listbox.setItemRenderer(new ListitemRenderer<Vpipelineunit>() {

			public void render(Listitem item, final Vpipelineunit data, int index) throws Exception {
				...
				BindUtils.postNotifyChange(null, null, PipelineSumUnitVm.this, "*");
			}
		});
	...
	public void doRefreshModel() {		
		try {
			objList = new TpipelineDAO().getSumByUnit("0=0");
			listbox.setModel(new ListModelList<Vpipelineunit>(objList));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void doExport() {
		try {
			...
			XSSFSheet sheet = workbook.createSheet("Summary Data Pipeline By Unit");
			...
			cell.setCellValue("Summary Data Pipeline By Unit");
			
			row = sheet.createRow(rownum++);

			Map<Integer, Object[]> datamap = new TreeMap<Integer, Object[]>();
			datamap.put(1,
					new Object[] { "No", "Institusi", "Unit", "Total Debitur", "TFP", "Total Credit", "Total Self Portion", "Total Fee" });
			no = 2;
			for (Vpipelineunit data : objList) {

				datamap.put(no,
						new Object[] { no - 1, data.getIsbumn().equals("Y") ? "BUMN" : "Non BUMN", data.getUnitcode(),
			...
			String filename = "PipelineByUnit_" + new SimpleDateFormat("yyMMddHHmm").format(new Date()) + ".xlsx";
	...
	public Vpipelineunit getObjForm() {
		return objForm;
	}

	public void setObjForm(Vpipelineunit objForm) {
