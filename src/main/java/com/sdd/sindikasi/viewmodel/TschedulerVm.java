...
import com.sdd.sindikasi.dao.TschedulerDAO;
import com.sdd.sindikasi.domain.Muser;
import com.sdd.sindikasi.domain.Tscheduler;
...
public class TschedulerVm {
		...
		if (grid != null) {
			grid.setRowRenderer(new RowRenderer() {
				...
				public void render(Row row, Object data, int index) throws Exception {
					...
					final Combobox cboxStatus = new Combobox(
							oForm.getSchedulerstatus().equals(AppUtils.SCHEDULER_DISABLE_VALUE)
									? AppUtils.SCHEDULER_DISABLE_LABEL
									: AppUtils.SCHEDULER_ENABLE_LABEL);
					...
					btnEdit.addEventListener(Events.ON_CLICK, new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							Messagebox.show(Labels.getLabel("common.update.confirm"), "Confirm Dialog",
										...
										public void onEvent(Event event) throws Exception {
											if (event.getName().equals("onOK")) {
												try {
													...
													oForm.setSchedulerstatus(cboxStatus.getValue()
															.equals(AppUtils.SCHEDULER_DISABLE_LABEL)
																	? AppUtils.SCHEDULER_DISABLE_VALUE
																	: AppUtils.SCHEDULER_ENABLE_VALUE);
													...
													Clients.showNotification(Labels.getLabel("common.update.success"),
													...
													String schedule = "";
													if (oForm.getSchedulerrepeattype()
															.equals(AppUtils.SCHEDULER_REPEAT_PERMINUTE)) {
														schedule = "0 0/" + oForm.getRepeatinterval() + " * * * ?";
													} else if (oForm.getSchedulerrepeattype()
															.equals(AppUtils.SCHEDULER_REPEAT_ATHOUR)) {
														schedule = "0 0 " + oForm.getRepeatinterval() + " * * ?";
													}

													Trigger oldTrigger = scheduler.getTrigger(new TriggerKey(
															oForm.getSchedulername(), oForm.getSchedulergroup()));
													TriggerBuilder triggerBuilder = oldTrigger.getTriggerBuilder();
													Trigger newTrigger = triggerBuilder
															.withIdentity(oForm.getSchedulername(),
																	oForm.getSchedulergroup())
															.withSchedule(CronScheduleBuilder.cronSchedule(schedule))
															.build();
													scheduler.rescheduleJob(oldTrigger.getKey(), newTrigger);

													if (oForm.getSchedulerstatus()
															.equals(AppUtils.SCHEDULER_DISABLE_VALUE))
														scheduler.pauseTrigger(new TriggerKey(oForm.getSchedulername(),
																oForm.getSchedulergroup()));
													else
														scheduler.resumeTrigger(new TriggerKey(oForm.getSchedulername(),
																oForm.getSchedulergroup()));
	...
	public void refreshModel() {
		try {
			grid.setModel(new ListModelList<Tscheduler>(new TschedulerDAO().list()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
